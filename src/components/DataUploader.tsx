...
export const DataUploader: React.FC<DataUploaderProps> = ({
  ...
  return (
    <div className="min-w-0 bg-[#18181B] border border-[#27272A] rounded-2xl p-3 sm:p-5 shadow-sm space-y-4">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
            ...
            {SAMPLE_DATASETS.map((sample) => {
              ...
              return (
                <button
                  ...
                  className={`max-w-full px-2.5 sm:px-3 py-1.5 rounded-lg text-xs font-medium border transition-all cursor-pointer ${
                    isSelected
        ...
        <div className="flex w-full flex-wrap items-stretch gap-2 self-start md:w-auto md:self-auto md:items-center md:shrink-0">
          <div
            role="button"
            tabIndex={0}
            ...
            className={`relative flex min-w-0 flex-[1_1_10rem] items-center justify-center gap-2 px-3 py-2 rounded-xl text-xs font-medium text-emerald-400 hover:text-white bg-emerald-500/10 border transition-all cursor-pointer md:flex-initial ${isDragActive ? 'border-emerald-400 bg-emerald-500/20' : 'border-emerald-500/20'}`}
          >
            <input
              ref={fileInputRef}
              type="file"
            ...
            <span className="truncate">{isUploading ? 'Uploading...' : isDragActive ? 'Drop bill here' : 'Drop or upload bill'}</span>
          </div>

          <button
            type="button"
            onClick={() => setShowCustomEditor(!showCustomEditor)}
            className="inline-flex min-w-0 flex-[1_1_9rem] items-center justify-center gap-1.5 px-3 py-2 rounded-xl text-xs font-medium text-[#A1A1AA] hover:text-white bg-[#09090B] border border-[#27272A] hover:border-zinc-700 transition-all cursor-pointer md:flex-initial"
          >
            <FileText className="w-3.5 h-3.5 text-blue-400" />
            <span className="truncate">{showCustomEditor ? 'Hide Editor' : 'Paste / Edit Statement'}</span>
          ...
          <button
            ...
            onClick={() => onAnalyze()}
            className="inline-flex min-w-0 flex-[1_1_9rem] items-center justify-center gap-2 px-3 sm:px-5 py-2 rounded-xl bg-blue-600 hover:bg-blue-500 active:bg-blue-700 text-white font-semibold text-xs shadow-sm transition-all disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer md:flex-initial"
          >
            {isLoading ? (
              <>
